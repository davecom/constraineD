...
void main() {
  ...
  int board_height = 3;
  //create the chips
  Chip a = new Chip("a", 3, 2);
  Chip b = new Chip("b", 5, 2);
  Chip c = new Chip("c", 2, 3);
  Chip e = new Chip("e", 7, 1);
  List variables = [a, b, c, e];
  Map domains = {};
  ...
  //add constraints
  cb_csp.addBinaryConstraint(new CircuitBoardConstraint(a, b));
  cb_csp.addBinaryConstraint(new CircuitBoardConstraint(a, c));
  cb_csp.addBinaryConstraint(new CircuitBoardConstraint(a, e));
  cb_csp.addBinaryConstraint(new CircuitBoardConstraint(b, c));
  cb_csp.addBinaryConstraint(new CircuitBoardConstraint(b, e));
  cb_csp.addBinaryConstraint(new CircuitBoardConstraint(c, e));

  //run the solution and calculate the time it took
  Stopwatch stopwatch = new Stopwatch()..start();
  Map solution = backtrackingSearch(cb_csp, {}, mrv: true);
